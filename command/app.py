...
def create_user(event, table):
    ...
    if "username" not in body:
        return {"statusCode": 400, "body": json.dumps({"code": "E_INVALID", "message": "username required"})}
    ...
    if check_user.get("Item", None):
        ...
        if str(is_active).strip() == "1":
            return {"statusCode": 400, "body": json.dumps({"code": "E_INVALID", "message": "user name already exist"})}
    ...
    if check_email.get("Count") > 0:
        ...
        if str(is_active).strip() == "1":
            return {"statusCode": 400, "body": json.dumps({"code": "E_INVALID", "message": "email already exist"})}
    ...
    try:
        ...
        return {"statusCode": 500, "body": json.dumps({"code": "error", "message": str(e)})}

    return {"statusCode": 200, "body": json.dumps({"code": "ok", "message": "create user"})}
...
def update_user(event, table):
    ...
    if "username" in body:
        return {"statusCode": 400, "body": json.dumps({"code": "E_INVALID", "message": "username not edit"})}
    ...
    return {"statusCode": 200, "body": json.dumps({"code": "ok", "message": "update user"})}
...
def delete_user(event, table):
    ...
    return {"statusCode": 200, "body": json.dumps({"code": "ok", "message": "delete user"})}
...
def create_group(event, table):
    ...
    if "groupname" not in body:
        return {"statusCode": 400, "body": json.dumps({"code": "E_INVALID", "message": "group_name required"})}
    ...
    if check_group.get("Item", None):
        ...
        if str(is_active).strip() == "1":
            return {"statusCode": 400, "body": json.dumps({"code": "E_INVALID", "message": "group name already exist"})}
    ...
    return {"statusCode": 200, "body": json.dumps({"code": "ok", "message": "create group"})}
...
def update_group(event, table):
    ...
    if "groupname" in body:
        return {"statusCode": 400, "body": json.dumps({"code": "E_INVALID", "message": "groupname not edit"})}

    if "description" not in body:
        return {"statusCode": 400, "body": json.dumps({"code": "E_INVALID", "message": "description required"})}
    ...
    return {"statusCode": 200, "body": json.dumps({"code": "ok", "message": "update group"})}
...
def delete_group(event, table):
    ...
    return {"statusCode": 200, "body": json.dumps({"code": "ok", "message": "delete group"})}
...
def add_group_member(event, table):
    ...
    if resp.get("Item", None) is not None:
        return {"statusCode": 500, "body": json.dumps({"code": "E_INVALID", "message": "user already exist in group"})}
    params["sso_type"] = "keycloak"
    params["updated_at"] = int(time.time())
    table.put_item(Item=params)
    return {"statusCode": 200, "body": json.dumps({"code": "ok", "message": "add group member"})}
...
def delete_group_member(event, table):
    ...
    if check_member.get("Item", None) is None:
        return {"statusCode": 400, "body": json.dumps({"code": "E_INVALID", "message": "user not in group"})}

    resp = table.delete_item(
        Key={"id": f"group#{group_id}", "sk": f"member#{user_id}"})
    return {"statusCode": 200, "body": json.dumps({"code": "ok", "message": "delete member group"})}
